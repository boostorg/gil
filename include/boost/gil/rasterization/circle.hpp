...
void rasterize_circle_trig(std::ptrdiff_t radius, point_t offset, RAIterator d_first)
{
    // const auto diameter = radius * 2 - 1;
    ...
    const std::size_t iteration_count = estimate_circle_point_count_trig(radius) / 8;
    double angle = 0;
    // do note that + 1 was done inside count estimation, thus <= is not needed, only <
    for (std::size_t i = 0; i < iteration_count; ++i, angle += minimum_angle_step)
    {
...
inline std::size_t estimate_circle_point_count_midpoint(std::ptrdiff_t radius)
{
...
void rasterize_circle_midpoint(std::ptrdiff_t radius, point_t offset, RAIterator d_first)
{
    ...
    std::ptrdiff_t iteration_distance = estimate_circle_point_count_midpoint(radius) / 8;
