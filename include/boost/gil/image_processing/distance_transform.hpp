...
template <typename DistanceType>
struct dt_parameters_are_compatible<DistanceType, mask_size::not_applicable_t> : std::false_type
{
};

template <typename MaskSize>
struct dt_parameters_are_compatible<distance_type::precise_euclidean_t, MaskSize> : std::false_type
{
};
...
void distance_transform_mask_size_three_impl(
...
{
    int const padding = 1;
    gray32f_image_t intermediate_image(
        src_view.width() + 2 * padding, src_view.height() + 2 * padding);
    ...
    for (std::ptrdiff_t y = src_view.height() - 1; y >= 0; --y)
    {
        for (std::ptrdiff_t x = src_view.width() - 1; x >= 0; --x)
        {
            if (intermediate_image_view(x + padding, y + padding)[0] > 0)
            {
                ...
                intermediate_image_view(x + padding, y + padding)[0] = std::min(
                    {d1,
                     d2,
                     d3,
                     d4,
...
/// Algorithm from - Principles of Digital Image Processing: Core Algorithms (Section 11.2.2).
template <typename SrcView, typename DstView, typename DistanceType>
void distance_transform_mask_size_five_impl(
    SrcView const& src_view,
    DstView const& dst_view,
    distance_from dist_from,
    DistanceType /*Passes type*/)
    ...
    std::ptrdiff_t n)
{
    ...
    for (std::ptrdiff_t q = 1; q < n; ++q)
    {
        float s = static_cast<float>(
            ((f[q][0] + std::pow(q, 2)) - (f[v[k]][0] + std::pow(v[k], 2))) / (2 * q - 2 * v[k]));
        while (s <= z[k])
        {
            k = k - 1;
            s = static_cast<float>(
                ((f[q][0] + std::pow(q, 2)) - (f[v[k]][0] + std::pow(v[k], 2)))
                / (2 * q - 2 * v[k]));
        }
        k = k + 1;
        v[k] = q;
...
void distance_transorm_precise_impl(
    SrcView const& src_view,
    DstView const& dst_view,
    distance_from dist_from)
{
    ...
    for (std::ptrdiff_t y = 0; y < src_view.height(); ++y)
    {
        for (std::ptrdiff_t x = 0; x < src_view.width(); ++x)
        {
            ...
            {
                if (src_view(x, y)[0] == src_channel_min)
                    intermediate_image_view(x, y)[0] = 0;
                else
                    intermediate_image_view(x, y)[0] = dt_infinite;
            }
            else
            {
                if (src_view(x, y)[0] != src_channel_min)
...
void distance_transform(
    ...
    MaskSize /*Passes type*/)
{
    gil_function_requires<ImageViewConcept<SrcView>>();
    gil_function_requires<MutableImageViewConcept<DstView>>();

    static_assert(
        std::is_same<gray_t, typename color_space_type<SrcView>::type>::value
    ...
    static_assert(
        detail::dt_check_distance_type<DistanceType>::value, "Distance type not recognized.");
    ...
    static_assert(
        detail::dt_parameters_are_compatible<DistanceType, MaskSize>::value,
