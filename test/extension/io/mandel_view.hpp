...
struct mandelbrot_fn
{
    ...
    result_type operator()(point_t const& p) const
    {
        ...
        double t = get_num_iter(point<double>(p.x / (double)img_size_.x * 3 - 2, p.y / (double)img_size_.y * 3 - 1.0f)); //1.5f));
        t = pow(t, 0.2);

        value_type ret;
        for (int k = 0; k < num_channels<P>::value; ++k)
            ret[k] = (typename channel_type<P>::type)(in_color_[k] * t + out_color_[k] * (1 - t));
        return ret;
    }
    ...
    -> typename mandel_view<Pixel>::type
{
    ...
    return view_t(dims, locator_t(point_t(0, 0), point_t(1, 1), deref_t(dims, in, out)));
}
