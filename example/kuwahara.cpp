...
void kuwahara_filter(SrcView const& src_view, DstView const& dst_view, std::size_t kernel_size)
{
    std::size_t half_kernel_size = kernel_size / 2;
		std::size_t a_val = kernel_size = (kernel_size - 1) / 2; // 2a + 1 = kernel_size
    ...
    for (std::ptrdiff_t y = 0; y < src_view.height(); y++)
    {
        ...
        for (std::ptrdiff_t x = 0; x < src_view.width(); x++)
        {
						...
						auto min_idx = std::min_element(std_deviations.begin(), std_deviations.end());
