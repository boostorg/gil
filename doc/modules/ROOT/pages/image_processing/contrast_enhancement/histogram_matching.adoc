...
. If the cumulative histogram of input image is H(p~x~) and of reference image is R(p~x'~)
  p~x~ & p~x'~ in [0, 255], then apply the transformation
  p~x'~ = R^-1^(H(p~x~))
...
image::https://github.com/boost-gil/test-images/blob/master/jpeg/suite/matching.jpg[Could not load image.]

*Histogram matched Image*

image::https://github.com/boost-gil/test-images/blob/master/jpeg/suite/matching_out.jpg[Could not load image.]
