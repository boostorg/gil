...
In the following discussion an _image_ is a 2D array of pixels. A _pixel_ is a
set of color channels that represents the color at a given point in an image.
Each _channel_ represents the value of a color component. There are two common
memory structures for an image. _Interleaved_ images are represented by
grouping the pixels together in memory and interleaving all channels together,
whereas _planar_ images keep the channels in separate color planes. Here is a
